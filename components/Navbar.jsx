...
export default function Navbar() {
  ...
  useEffect(() => {
    if (settings.server_ip) {
      fetch(`https://api.mcsrvstat.us/3/${settings.server_ip}`)
        .then((res) => res.json())
        .then((data) => {
        ...
        <header>
          ...
          >
            ...
            <Group gap="2rem" visibleFrom="md">
              <Group>
                <LanguageSwitcher />
                {!loading && user && <Basket user={user} />}
                {!loading && (
                  <Button
                    bg="primary"
                    h="3.1rem"
                    leftSection={<TbUserFilled />}
                    size="lg"
                    variant="login"
              ...
              <JoinButton>
                <Button className="play-button" size="lg">
                  <Stack align="center" gap={5}>
                    <Text mb="-0.2rem" fw={700}>
                      <NumberFormatter prefix="" value={onlinePlayers} />{" "}
                      {t("online")}
                    </Text>
                    <Group gap="0.2rem">
                      <TbCopy size="0.9rem" />
                      <Text mb="-2px" size="xs" mt="-0.2rem" fw={700}>
                        {settings.server_ip}
                      </Text>
                    </Group>
                  </Stack>
            ...
            {/* RIGHT: mobile actions (cart + login) + burger */}
            <Group hiddenFrom="md" gap="0.6rem" style={{ zIndex: 10 }}>
              <LanguageSwitcher />
